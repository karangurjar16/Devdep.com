...
import { spawnCommand, spawnInDirectoryWithEnv, execCommand, chainCommands, buildCdCommand } from "./config/commandRunner";
...
export async function startWithPM2(
  ...
  try {
    ...
    try {
      ...
      try {
        await spawnCommand("pm2", ["resurrect"]);
    ...
    try {
      await spawnCommand("pm2", ["delete", name]);
    ...
    if (entryFile === 'NEXT_START') {
      ...
      result = await spawnInDirectoryWithEnv(
        projectPath,
        { PORT: port.toString(), NODE_ENV: "production" },
        "pm2",
        ["start", entryFile, "--name", name]
      );
    }
...
export async function stopPM2Process(id: string): Promise<{ status: string; process: string; error?: string }> {
  try {
    ...
    try {
      const result = await spawnCommand("pm2", ["delete", id]);
